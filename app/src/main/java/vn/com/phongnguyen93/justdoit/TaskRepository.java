...
public class TaskRepository extends SQLiteOpenHelper implements RepositoryInterface {
  ...
  private ArrayList<Task> getAllItemByDate(String date) {
    ArrayList<Task> tasks = new ArrayList<>();
    SQLiteDatabase db = getReadableDatabase();

    String SQL_QUERY_TASKS_BY_DATE =
        String.format("SELECT * FROM %s WHERE %s LIKE '%s'", TABLE_TASKS, KEY_TASK_DATE, date);
  ...
  public ArrayList<String> getAllItemDate() {
    ...
    String SQL_QUERY_TASKS_BY_DATE =
        String.format("SELECT %s FROM %s ", KEY_TASK_DATE, TABLE_TASKS);
  ...
  @Override public ArrayList<Task> getTasks(String date) {
    return getAllItemByDate(date);
  }
